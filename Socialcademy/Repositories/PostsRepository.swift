...
protocol PostsRepositoryProtocol {
    ...
    func fetchPosts(from author: User) async throws -> [Post]
}
...
struct PostsRepository: PostsRepositoryProtocol {
    ...
    func create(_ post: Post) async throws {
        ...
        if let imageFileURL = post.imageURL {
            post.imageURL = try await StorageFile.with(namespace: "posts", identifier: post.id.uuidString).putFile(from: imageFileURL).getDownloadURL()
        }
...
private extension PostsRepository {
    ...
    func fetchFavorites() async throws -> [Post.ID] {
        return try await favoritesReference.whereField("userID", isEqualTo: user.id).getDocuments(as: Favorite.self).map((\.postID))
    }
    
    func fetchPosts(from query: Query) async throws -> [Post] {
        let (posts, favorites) = try await (query
            .order(by: "timestamp", descending: true)
            .getDocuments(as: Post.self),
                                            fetchFavorites())
